...
if (!isLib) {
  ...
  const knowledge_map_styles = fs.readFileSync(
...
const inlineJpegAssets = () => ({
  name: "inline-jpeg-assets",
  load(id) {
    if (!/\.jpe?g$/i.test(id)) {
      return null;
    }
    const encoded = fs.readFileSync(id).toString("base64");
    return `export default "data:image/jpeg;base64,${encoded}";`;
  },
});
