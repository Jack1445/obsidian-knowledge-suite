  ...
  implements HoverParent
{
  ...
  /**
   * Monotonically increases whenever the view starts loading/reloading a file.
   * Obsidian can reuse one Excalidraw view for another file, so async work must
   * prove that it still belongs to the file/load that started it before it may
   * update the scene or save to disk.
   */
  private viewLoadGeneration = 0;
  private viewLoadQueue: Promise<void> = Promise.resolve();

  private isCurrentViewLoad(file: TFile | null, generation: number): boolean {
    return isCurrentViewLoadIdentity(
      this.file,
      file,
      this.viewLoadGeneration,
      generation,
    );
  }

  private isStableViewForFile(file: TFile | null, generation: number): boolean {
    return isStableViewForFileIdentity(
      this.file,
      file,
      this.viewLoadGeneration,
      generation,
      this.lastLoadedFile,
      this.excalidrawData?.file ?? null,
    );
  }
  ...
  async save(
    ...
    const saveGeneration = this.viewLoadGeneration;
    const isSaveCurrent = (): boolean =>
      guardAllowsSave() &&
      this.isStableViewForFile(saveFile, saveGeneration);
    if (!isSaveCurrent()) {
      return;
    }
    ...
    if (
      ...
      !saveFile ||
      !isSaveCurrent() ||
      !this.app.vault.getAbstractFileByPath(saveFile.path) //file was recently deleted
    ) {
      abortSave();
      return;
    }
    ...
    try {
      if (allowSave) {
        ...
        if (this.compatibilityMode) {
          await this.excalidrawData.syncElements(scene, undefined, isSaveCurrent);
          if (!isSaveCurrent()) {
            abortSave();
            return;
          }
        } else {
          const shouldReloadDrawing = await this.excalidrawData.syncElements(
            scene,
        ...
        triggerReload =
          this.lastSaveTimestamp === saveFile.stat.mtime &&
          !preventReload &&
          forcesave;
        this.lastSaveTimestamp = saveFile.stat.mtime;
      ...
      // If an asynchronous save step failed after clearing the dirty marker,
      // keep the document dirty so the next save can retry it. This is also
      // identity-safe: abortSave only restores the marker for the same file
      // and load generation that started this save.
      abortSave();
    }
    this.semaphores.saving = false;
    if (triggerReload && isSaveCurrent()) {
      await this.reload(true, saveFile);
    }
  ...
  async prepareGetViewData(isCurrent?: () => boolean): Promise<void> {
    const guardAllowsPreparation = (): boolean => {
      try {
        return isCurrent?.() !== false;
      } catch {
        return false;
    ...
    if (!this.compatibilityMode) {
      ...
      const excalidrawData = this.excalidrawData;
      const deletedElements = this.excalidrawAPI
        .getSceneElementsIncludingDeleted()
        .filter((el: ExcalidrawElement) => el.isDeleted);
      const generatedScene = IS_WORKER_SUPPORTED
        ? await excalidrawData.generateMDAsync(deletedElements)
        : excalidrawData.generateMDSync(deletedElements);

      excalidrawData.disableCompression = false;
      if (!guardAllowsPreparation()) {
        return;
      }
      this.viewSaveData = header + generatedScene + tail;
      return;
    }
    if (this.compatibilityMode) {
      if (guardAllowsPreparation()) {
  ...
  async onUnloadFile(): Promise<void> {
    ...
    const unloadFile = this.file;
    const unloadGeneration = this.viewLoadGeneration;
    ...
    await this.forceSaveIfRequired(unloadFile, unloadGeneration);
  }

  private async forceSaveIfRequired(
    expectedFile: TFile | null = this.file,
    expectedGeneration: number = this.viewLoadGeneration,
  ): Promise<boolean> {
    let watchdog = 0;
    let dirty = false;
    ...
    if (
      !this.excalidrawAPI ||
      !this.isStableViewForFile(expectedFile, expectedGeneration)
    ) {
      return false;
    }
    ...
    if (
      ...
      this.checkSceneVersion(this.excalidrawAPI.getSceneElements());
      if (this.isDirty()) {
        const path = expectedFile?.path;
        const plugin = this.plugin;
        window.setTimeout(() => {
          plugin.triggerEmbedUpdates(path);
        }, 400);
        dirty = true;
        if (this.isStableViewForFile(expectedFile, expectedGeneration)) {
          await this.save(
            true,
            true,
            true,
  ...
  public async reload(fullreload: boolean = false, file?: TFile) {
    ...
    const api = this.excalidrawAPI;
    if (!api || !this.isCurrentViewLoad(reloadFile, reloadGeneration)) {
      return;
    }

    if (loadOnModifyTrigger) {
      this.data = await this.app.vault.read(reloadFile);
      if (!this.isCurrentViewLoad(reloadFile, reloadGeneration)) {
        return;
      }
      this.preventAutozoom();
    }
    if (fullreload) {
      await this.excalidrawData.loadData(this.data, reloadFile, this.textMode);
    } else {
    ...
    if (this.isCurrentViewLoad(reloadFile, reloadGeneration)) {
      this.lastLoadedFile = reloadFile;
      this.clearDirty();
    }
  }
  ...
  setViewData(data: string, clear: boolean = false) {
    ...
    const loadTask = async (): Promise<void> => {
      ...
      if (isMaskFile(this.plugin, requestedFile)) {
      ...
      this.lastSaveTimestamp = requestedFile.stat.mtime;
      ...
      await new Promise<void>((resolve) => {
        ...
        if (!this.isCurrentViewLoad(requestedFile, loadGeneration)) {
          return;
        }
        this.compatibilityMode = requestedFile.extension === "excalidraw";
        ...
        if (this.compatibilityMode) {
          ...
          await this.excalidrawData.loadLegacyData(data, requestedFile);
          if (!this.isCurrentViewLoad(requestedFile, loadGeneration)) {
            return;
          }
        ...
        if (
          ...
          const backup = await getImageCache().getBAKFromCache(requestedFile.path);
          if (!this.isCurrentViewLoad(requestedFile, loadGeneration)) {
            return;
          }
            ...
            window.setTimeout(() => {
              void (async () => {
                ...
                if (result === 1) {
                  const path = getNewUniqueFilepath(
                    this.app.vault,
                    `${requestedFile.basename}.restored.${requestedFile.extension}`,
                    requestedFile.parent.path,
                  );
                  ...
                  await getImageCache().removeBAKFromCache(requestedFile.path);
        ...
        if (script) {
          const scriptname = `${requestedFile.basename}-onlaod-script`;
    ...
    this.viewLoadQueue = this.viewLoadQueue.then(loadTask, loadTask);
  }
  ...
  public async loadDrawing(
    ...
    const file = expectedFile ?? this.file;
    if (!file) {
      return;
    }
    const excalidrawData = this.excalidrawData.scene;
    const isOpenInMultipleLeaves =
      getExcalidraAndMarkdowViewsForFile(this.app, file).length > 1;
  ...
  public async addElements({
    ...
    const addGeneration = this.viewLoadGeneration;
    const excalidrawData = this.excalidrawData;
    const hasAddGuard = typeof isCurrent === "function";
    const getAddData = (): ExcalidrawData =>
      hasAddGuard ? excalidrawData : this.excalidrawData;
    const isAddCurrent = (): boolean => {
      if (!hasAddGuard) {
        return true;
      }
      let guardAllowsAdd = true;
      try {
        guardAllowsAdd = isCurrent() !== false;
      } catch {
        guardAllowsAdd = false;
      }
      return Boolean(
        guardAllowsAdd &&
          api &&
          api === this.excalidrawAPI &&
          excalidrawData === this.excalidrawData &&
          this.isCurrentViewLoad(addFile, addGeneration),
      );
    };
    if (!api || !isAddCurrent()) {
      return false;
    ...
    for (let i = 0; i < textElements.length; i++) {
      ...
      const { parseResult, link } = await getAddData().addTextElement(
        textElement.id,
        textElement.text,
        textElement.rawText, //TODO: implement originalText support in ExcalidrawAutomate
        hasAddGuard ? isAddCurrent : undefined,
      );
      if (!isAddCurrent()) {
        return false;
      }
    ...
    if (images && Object.keys(images).length > 0) {
      Object.keys(images).forEach((k: FileId) => {
        ...
        if (images[k].file || images[k].isHyperLink) {
          ...
          getAddData().setFile(images[k].id, embeddedFile);
        ...
        if (images[k].latex) {
          getAddData().setEquation(images[k].id, {
    ...
    if (save) {
      await this.save(
        ...
        false,
        hasAddGuard ? isAddCurrent : undefined,
      ); //preventReload=false will ensure that markdown links are paresed and displayed correctly
      if (!isAddCurrent()) {
        return false;
      }
    } else {
  ...
  private onBeforeTextEdit(
    ...
    const elementRawText = textElement.rawText ?? textElement.originalText;
    const raw = this.excalidrawData.getRawText(textElement.id);
    const hasPersistedInlineFormula =
      !!textElement.customData?.obsidianInlineFormulas &&
      /\\\(.+?\\\)/.test(elementRawText);
    if (hasPersistedInlineFormula && raw !== elementRawText) {
      // Inline formulas are committed directly by the forked Core because the
      // shared modal closes Excalidraw's textarea. Drawings created before the
      // host-cache synchronization fix may still contain a stale # Text
      // Elements entry. Prefer the element's persisted raw source and repair
      // the cache before opening the native editor.
      this.excalidrawData.setTextElement(
        textElement.id,
        elementRawText,
        () => {},
      );
      return elementRawText;
    }
    if (!raw) {
      return elementRawText;
    }
