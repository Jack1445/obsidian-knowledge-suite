...
import { getEnabledLegacyPluginNames } from "./legacyPluginConflict";
...
export default class ExcalidrawPlugin extends Plugin {
  ...
  constructor(app: App, manifest: PluginManifest) {
    super(app, manifest);
    this.dataCoordinator = new KnowledgeSuiteDataCoordinator(this);
  ...
  async onload() {
    ...
    const enabledLegacyPlugins = getEnabledLegacyPluginNames(this.app);
    if (enabledLegacyPlugins.length > 0) {
      this.initializationBlocked = true;
      const message = t("LEGACY_PLUGIN_CONFLICT").replace(
        "{PLUGINS}",
        enabledLegacyPlugins.join(", "),
      );
    ...
    try {
      const migration = await this.dataCoordinator.importLegacyData(this.app);
      if (
        !migration.alreadyCompleted &&
        Object.values(migration.sources).some(
          (source) => source.status === "imported",
        )
      ) {
        new Notice(t("LEGACY_DATA_IMPORT_SUCCESS"), 8000);
      }
    } catch (error) {
      this.initializationBlocked = true;
      new Notice(t("LEGACY_DATA_IMPORT_FAILED"), 15000);
      console.error("Knowledge Suite legacy data import failed", error);
      return;
    }
    ...
    try {
      this.knowledgeMap = new KnowledgeMapController(
        this,
        this.dataCoordinator.getNamespace<KnowledgeMapData>("knowledgeMap"),
      );
      await this.knowledgeMap.initialize();
    } catch (error) {
  ...
  public async loadSettings(
    ...
    const rawSettings = ((await this.dataCoordinator.excalidraw.loadData()) ??
    ...
    if (didMigrate || didSettingsMigration || shouldPersistEncryptedSettings) {
      await this.dataCoordinator.excalidraw.saveData(encryptedPersistedSettings);
    }
  ...
  async saveSettings() {
    ...
    await this.dataCoordinator.excalidraw.saveData(
      encryptPersistedAPIKeys(persistedSettings),
    );
  }
  ...
  /**
   * Reuses Knowledge Map's established formula editor and KaTeX renderer so
   * inline and global formulas have exactly the same editing experience.
   */
  public async editInlineFormula(initialLatex?: string): Promise<{
    latex: string;
    dataURL: string;
    width: number;
    height: number;
    ...
    try {
      if (!this.knowledgeMap) {
        new Notice(t("KNOWLEDGE_MAP_NOT_READY"));
        return null;
      }
      return await this.knowledgeMap.editInlineFormula(initialLatex ?? "");
    } catch {
      return null;
    }
  }
  ...
  public async renderInlineFormula(latex: string): Promise<{
    ...
    try {
      return this.knowledgeMap
        ? await this.knowledgeMap.renderInlineFormula(latex)
        : null;
    } catch {
      return null;
    }
