...
export class ExcalidrawIntegration {
	...
	get available(): boolean {
		return Boolean(getExcalidrawAutomate());
	}

	isDrawing(file: TFile): boolean {
		return getExcalidrawAutomate()?.isExcalidrawFile?.(file) ?? file.path.endsWith('.excalidraw.md');
	}
	...
	private requireApi(showNotice = true): ExcalidrawAutomateLike | null {
		const api = getExcalidrawAutomate();
		if (api) return api;
