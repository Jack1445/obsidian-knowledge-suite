...
- 精简全局设置页新增默认收起的“默认路径/命名设置”：可分别设置以后新建的2维与3维管理画布名称和保存路径，四项留空时严格沿用原有行为，且不会修改已有画布。
- 将两个重复的 Knowledge Suite 全局设置页替换为一个无开关的精简说明页，不再展示继承自 Excalidraw 的 AI、OCR、脚本、发布、本地字体、移动端、宣传更新和实验性设置；Knowledge Map 的可视参数继续在图谱工具栏就地调整。底层格式兼容、强制备份和旧数据安全导入仍保留且不可关闭。
- 新增独立的语义筛选画布：只通过绑定 Markdown 的标签与属性动态筛选语义单位，以高保真只读预览展示，并仅允许临时整体移动；不创建同步实例，也不修改来源画布。
- 语义筛选画布的单位卡片改为按原始内容边界自适应放大：大单位以接近 1:1 的尺寸显示，小单位保留易读的最小尺寸，同时继续保留卡片边框与自动防重叠布局。
- 语义筛选画布中的单位卡片现可通过右键选择、更换或解除依托 Markdown；解除关系不会删除 Markdown、画布内容或同步实例。
- 普通 Excalidraw 二维画布、Knowledge Map 二维视图和语义筛选画布的默认最小缩放统一放宽至 5%，便于总览大型画布；已有自定义最小缩放设置不会被覆盖，三维视图不受影响。
- 重做语义单位筛选弹窗：压缩标题与说明，使用“全部 / 任一”分段选择、条件计数、编号条件卡片和明确的空状态入口，并统一紫色强调与按钮层级。
- 语义单位筛选弹窗中的次要操作按钮统一改为白底细边框，减少灰色块带来的厚重感，同时保留紫色主操作与选中状态。
- 语义单位筛选条件中的字段和运算符下拉框也统一使用白色背景，与输入框和次要按钮保持一致。
- 语义筛选画布新增按住鼠标中键拖动平移；保留空白处左键拖动，并允许中键从语义单位卡片上直接开始平移，以对齐普通二维画布习惯。
